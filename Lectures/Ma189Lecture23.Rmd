...
```{r}
set.seed(777)
test_ind <- sample(392,size=196)
val_ind <- setdiff(seq(1,392),test_ind)
...
- Testing error may decrease as a function of model complexity up to a point; balance over-fitting (high model order) versus under-fitting (low model order).
- Here, Model 2 has the best testing error.
- The model that minimizes training error (Model 5) is more complicated than the model that minimizes testing error (Model 2).
...
out <- rbind(out,test_err)
}

plot.ts(t(out),plot.type="single",col=seq(1,10),
        xlab="Model Number",ylab="Testing Error")
```
