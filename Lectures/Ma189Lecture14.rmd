...
- For each of the $30$ test records $\underline{x}$, we can plot $\widehat{d}_k^L (\underline{x})$ for $k = 1, 2, 3$. These are plotted on axes for setosa ($k=1$), versicolor ($k=2$), and virginica ($k=3$).
- In the second scatterplot, we color the records according to their true species: red square for setosa, blue triangle for versicolor, and black dot for virginica. Observe the clustering of discriminant values.
...
- The classification results versus truth are given in the following output:
...
```{r}
class_tab <- c(dim(iris.setosa)[1]-n_setosa,
               dim(iris.versi)[1]-n_versicolor,
               dim(iris.virgi)[1]-n_virginica)
...
- Letting $c = \frac{1}{2} \left( \underline{\mu}_1^{\prime} {\mathbf \Sigma}^{-1} \underline{\mu}_1  - \underline{\mu}_2^{\prime} {\mathbf \Sigma}^{-1} \underline{\mu}_2 \right) - \log (p_1 /p_2)$ and $\underline{\omega} = {\mathbf \Sigma}^{-1} {( \underline{\mu}_1 - \underline{\mu}_2 )}$,
